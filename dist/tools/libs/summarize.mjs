...
*/
var COMPACT_CONTENT_MIN_LENGTH = 500;
function renderTable(section) {
	...
	for (const row of rows) {
		html += "<tr>";
		for (const cell of row) if (typeof cell === "string") html += `<td>${cell}</td>`;
		else {
...
function renderDetails(section) {
	const content = stripAnsi$1(section.content);
	return `<details${section.open ? " open" : ""}><summary>${section.summary}</summary>${content}</details>\n`;
}
function renderList(section) {
	const tag = section.ordered ? "ol" : "ul";
	return `<${tag}>${section.items.map((item) => `<li>${stripAnsi$1(item)}</li>`).join("")}</${tag}>\n`;
}
function renderCode(section) {
	const content = stripAnsi$1(section.content);
	return `<pre${section.language ? ` lang="${section.language}"` : ""}><code>${content}</code></pre>\n`;
}
...
function renderQuote(section) {
	const text = stripAnsi$1(section.text);
	return `<blockquote${section.cite ? ` cite="${section.cite}"` : ""}><p>${text}</p></blockquote>\n`;
}
function renderRaw(section) {
	return stripAnsi$1(section.content) + "\n";
}
function renderSection(section) {
	switch (section.type) {
		case "table": return renderTable(section);
		case "details": return renderDetails(section);
		...
		case "raw": return renderRaw(section);
...
function isCompactEligible(section, sectionHtml) {
	if (section.type === "separator" || section.type === "table" || section.type === "heading" || section.type === "quote") return false;
	if ("critical" in section && section.critical === true) return false;
	return sectionHtml.length > COMPACT_CONTENT_MIN_LENGTH;
}
function getCompactLabel(section) {
	if (section.type === "details") return section.summary;
	if (section.type === "list" || section.type === "code") return section.heading ?? section.type;
	return section.type;
}
function wrapInDetails(label, innerHtml) {
...
var SummaryService = class {
	...
	overwrite;
	agent;
	constructor(payload, compact, compactThreshold, overwrite, agent) {
	...
	async write() {
		...
		const headerHtml = status ? `<h2>${STATUS_EMOJI[status]} ${title}</h2>\n` : `<h2>${title}</h2>\n`;
		...
		if (this.compact && fullHtml.length > this.compactThreshold) {
			const compactedParts = [headerHtml];
			for (const [i, section] of sections.entries()) {
				const sectionHtml = renderedSections[i];
				...
				} else compactedParts.push(sectionHtml);
			}
		...
		return {
			characterCount: finalHtml.length,
			wasCompacted
		};
	}
};
//#endregion
//#region src/actions/summarize/SummaryBuilder.ts
var SummaryBuilder = class SummaryBuilder {
	_payload;
	_compact = true;
	_compactThreshold = 9e5;
	_overwrite = true;
	_agent;
	constructor() {}
	static create() {
	...
	withCompactThreshold(threshold) {
	...
	build() {
		if (!this._payload) throw new Error("SummaryBuilder: payload is required");
		if (!this._payload.title || this._payload.title.trim() === "") throw new Error("SummaryBuilder: payload.title must be a non-empty string");
		if (this._compactThreshold <= 0) throw new Error("SummaryBuilder: compactThreshold must be greater than 0");
		if (!this._agent) throw new Error("SummaryBuilder: agent is required");
		return new SummaryService(this._payload, this._compact, this._compactThreshold, this._overwrite, this._agent);
	}
};
//#endregion
...
//#endregion
//#region src/actions/summarize/PayloadParser.ts
var ANSI_REGEX = /[\u001B\u009B][[\]()#;?]*(?:(?:(?:(?:;[-a-zA-Z\d/#&.:=?%@~_+]*)*|[a-zA-Z\d]+(?:;[-a-zA-Z\d/#&.:=?%@~_+]*)*)?\u0007)|(?:(?:\d{1,4}(?:;\d{0,4})*)?[\dA-PR-TZcf-nq-uy=><~]))/g;
function stripAnsi(input) {
	return input.replace(ANSI_REGEX, "");
}
var PayloadParseError = class extends Error {
	format;
...
var VALID_SECTION_TYPES = new Set([
	"table",
	"details",
	"list",
	"code",
...
function validatePayload(raw, format) {
	if (typeof raw["title"] !== "string" || raw["title"].trim() === "") throw new PayloadParseError(`"title" must be a non-empty string`, format);
	const title = raw["title"].trim();
	let status;
	let sections;
	if (raw["status"] !== void 0) {
		if (typeof raw["status"] !== "string" || !VALID_STATUSES.has(raw["status"])) throw new PayloadParseError(`"status" must be one of: ${[...VALID_STATUSES].join(", ")}`, format);
		status = raw["status"];
	}
	if (raw["sections"] !== void 0) {
		if (!Array.isArray(raw["sections"])) throw new PayloadParseError(`"sections" must be an array`, format);
		for (const section of raw["sections"]) if (!isRecord(section) || typeof section["type"] !== "string" || !VALID_SECTION_TYPES.has(section["type"])) throw new PayloadParseError(`Each section must have a "type" field with one of: ${[...VALID_SECTION_TYPES].join(", ")}`, format);
		sections = raw["sections"];
	}
	const result = { title };
	if (status !== void 0) result.status = status;
	if (sections !== void 0) result.sections = sections;
	return result;
}
...
var PayloadParser = class PayloadParser {
	static {
		new PayloadParser();
	}
	constructor() {}
	static parse(raw) {
		...
		if (cleaned.startsWith("{")) {
			...
			return {
				payload: validatePayload(parsed, "json"),
				format: "json"
			};
		}
		try {
			...
			if (isRecord(parsed)) {
				if (typeof parsed["title"] === "string" && parsed["title"].trim() !== "") return {
					payload: validatePayload(parsed, "yaml"),
					format: "yaml"
				};
			}
		} catch {}
...
//#endregion
//#region src/tools/summarize/settings.ts
/**
...
function getSettings(agent) {
	...
	return {
		...
		compactThreshold: compactThresholdRaw ? parseInt(compactThresholdRaw, 10) : 9e5,
		overwrite: agent.getBooleanInput("overwrite")
	};
}
//#endregion
//#region src/tools/summarize/runner.ts
/**
* Summarize runner
* Parses an input payload (JSON | YAML | Markdown) and writes
* a structured summary to the GitHub Step Summary panel.
*/
var SummarizeRunner = class extends RunnerBase {
	name = "summarize";
	steps = new Map([["write", this.runWrite.bind(this)]]);
	constructor() {
		super();
	}
	async runWrite(agent) {
		try {
			const settings = getSettings(agent);
			const { payload, format } = PayloadParser.parse(settings.payload);
			const service = SummaryBuilder.create().withPayload(payload).withCompact(settings.compact).withCompactThreshold(settings.compactThreshold).withOverwrite(settings.overwrite).withAgent(agent).build();
			...
			if (error instanceof PayloadParseError) return this.failure(error);
...
*/
function createSummarizeRunner() {
	return new SummarizeRunner();
}
//#endregion
export { createSummarizeRunner as t };
