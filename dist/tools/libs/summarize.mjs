...
function renderList(section) {
  ...
  const items = section.items.map((item) => `<li>${stripAnsi$1(item)}</li>`).join("");
...
function isCompactEligible(section, sectionHtml) {
  if (section.type === "separator" || section.type === "table" || section.type === "heading" || section.type === "quote") {
    return false;
  }
...
const VALID_SECTION_TYPES = /* @__PURE__ */ new Set([
  "table",
  "details",
  "list",
  "code",
...
class SummarizeRunner extends RunnerBase {
  name = "summarize";
...
export { assertNever as a, createSummarizeRunner as c };
