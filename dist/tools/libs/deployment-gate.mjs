...
import { a as warning, i as info, n as debug, r as error } from "./agents.mjs";
//#region src/tools/common/runner-base.ts
/**
	...
	async run(agent, step) {
		...
		if (!stepFn) {
			...
			return {
				success: false,
				outputs: {},
				error: /* @__PURE__ */ new Error(`Unknown step '${step}' for runner '${this.name}'. Available steps: ${availableSteps}`)
			};
		}
		try {
...
//#endregion
//#region src/libs/utils/arrays.ts
/**
...
//#endregion
//#region src/libs/utils/logger.ts
/**
...
var Logger = class Logger {
	component;
	defaultContext;
	constructor(component, defaultContext = {}) {
	...
	*/
	formatMessage(message, context) {
		...
		const contextPairs = Object.entries(mergedContext).filter(([, value]) => value !== void 0).map(([key, value]) => `${key}=${JSON.stringify(value)}`).join(" ");
		const componentPrefix = `component=${JSON.stringify(this.component)}`;
		return contextPairs ? `${componentPrefix} ${contextPairs} msg=${JSON.stringify(message)}` : `${componentPrefix} msg=${JSON.stringify(message)}`;
	}
	/**
	* Debug level logging
	*/
	...
	*/
	warn(message, context = {}) {
		warning(this.formatMessage(message, context));
	}
	/**
	* Error level logging
	*/
	error(message, context = {}) {
		error(this.formatMessage(message, context));
	}
};
/**
* Creates a logger instance for a specific component
*/
function createLogger$1(component, context = {}) {
...
*/
var logger$1 = {
	docker: createLogger$1("docker"),
	github: createLogger$1("github"),
	version: createLogger$1("version"),
	action: createLogger$1("action")
};
createLogger$1("action");
//#endregion
//#region src/libs/utils/constants.ts
/**
...
*/
var SIMPLE_SEMVER_REGEX = /^v?(\d+)\.(\d+)\.(\d+)((?:-[a-zA-Z0-9\-\\.]+)?(?:\+[a-zA-Z0-9\-\\.+]+)?)?$/;
var CONTROL_CHAR_REGEX = /[\x00-\x1F\x7F]/g;
//#endregion
//#region src/libs/utils/parsers.ts
/**
...
function parseJsonObject(input) {
	...
	try {
		...
		warning(`Failed to parse JSON: ${input}`);
		return {};
	}
}
logger$1.docker;
//#endregion
//#region src/libs/utils/ValidationUtils.ts
/**
...
var ValidationUtils = {
	...
	*/
	validateStringInput(input, fieldName = "Input") {
		if (ValidationUtils.isNullOrUndefined(input)) throw new Error(`${fieldName} cannot be null or undefined`);
	},
	/**
	* Validate a metadata key-value pair.
	* @throws Error if key or value is invalid
	*/
	validateMetaDataInput(key, value) {
	...
	*/
	validateCommand(command) {
		if (!command || command.trim() === "") throw new Error("Command cannot be empty or null");
	}
};
//#endregion
...
function withDefaults$3(oldDefaults, newDefaults) {
	...
	return Object.assign(endpoint2, {
		DEFAULTS: DEFAULTS2,
		defaults: withDefaults$3.bind(null, DEFAULTS2),
...
function withDefaults$2(oldEndpoint, newDefaults) {
	...
	const newApi = function(route, parameters) {
		...
		Object.assign(request2, {
			endpoint: endpoint2,
			defaults: withDefaults$2.bind(null, endpoint2)
		});
...
var request$1 = withDefaults$2(endpoint, defaults_default$1);
/* v8 ignore next -- @preserve */
...
//#endregion
//#region src/actions/deployment-gate/approval.ts
/**
* Default words that count as approval
*/
var DEFAULT_APPROVED_WORDS = [
	"approved",
	"approve",
	"lgtm",
	"yes"
...
*/
function matchesKeyword(body, words) {
	return words.some((word) => {
		return new RegExp(`^${escapeRegExp(word)}[.!]*\\s*$`, "i").test(body.trim());
	});
}
/**
...
*/
function evaluateComments(comments, approvers, minimumApprovals, approvedWords, deniedWords) {
	const requiredApprovals = minimumApprovals > 0 ? minimumApprovals : approvers.length;
	const approvedBy = [];
	for (const comment of comments) {
		if (!approvers.some((approver) => approver.toLowerCase() === comment.user.toLowerCase())) continue;
		if (matchesKeyword(comment.body, deniedWords)) return {
			status: ApprovalStatus.Denied,
			approvedBy,
			deniedBy: comment.user
		};
		if (matchesKeyword(comment.body, approvedWords)) {
			if (!approvedBy.some((u) => u.toLowerCase() === comment.user.toLowerCase())) approvedBy.push(comment.user);
			if (approvedBy.length >= requiredApprovals) return {
...
//#endregion
//#region src/actions/deployment-gate/services/DeploymentGateService.ts
/**
* Maximum characters per issue comment (GitHub limit is 65536)
*/
var MAX_COMMENT_LENGTH = 65536;
var DeploymentGateService = class {
	token;
	owner;
	repo;
	minimumApprovals;
	...
	failOnDenial;
	excludeWorkflowInitiator;
	additionalApprovedWords;
	additionalDeniedWords;
	octokit;
	_approvers;
	constructor(token, owner, repo, approvers, minimumApprovals, issueTitle, issueBody, pollingIntervalSeconds, failOnDenial, excludeWorkflowInitiator, additionalApprovedWords, additionalDeniedWords) {
		this.token = token;
		this.owner = owner;
		this.repo = repo;
		...
		this.failOnDenial = failOnDenial;
		this.excludeWorkflowInitiator = excludeWorkflowInitiator;
		this.additionalApprovedWords = additionalApprovedWords;
		this.additionalDeniedWords = additionalDeniedWords;
	...
	async resolveApprovers(org, excludeActor) {
		const resolved = [];
	...
	async createApprovalIssue() {
		...
		if (this.issueBody) await this.postBodyAsComments(issue.number, this.issueBody);
		return {
			number: issue.number,
			url: issue.html_url
		};
	...
	async getIssueComments(issueNumber) {
		return (await this.octokit.paginate(this.octokit.issues.listComments, {
			...
			per_page: 100
		})).filter((c) => Boolean(c.user?.login && c.body)).map((c) => ({
			user: c.user.login,
			body: c.body
		}));
	}
	...
	evaluateApproval(comments) {
		const allApproved = [...DEFAULT_APPROVED_WORDS, ...this.additionalApprovedWords];
		const allDenied = [...DEFAULT_DENIED_WORDS, ...this.additionalDeniedWords];
		return evaluateComments(comments, this._approvers, this.minimumApprovals, allApproved, allDenied);
	}
	async postBodyAsComments(issueNumber, body) {
		const chunks = this.splitIntoChunks(body, MAX_COMMENT_LENGTH);
		for (const chunk of chunks) await this.octokit.issues.createComment({
			owner: this.owner,
			repo: this.repo,
			issue_number: issueNumber,
			body: chunk
...
//#endregion
//#region src/actions/deployment-gate/DeploymentGateBuilder.ts
/**
...
*/
var DeploymentGateBuilder = class DeploymentGateBuilder {
	_token;
	_owner;
	_repo;
	_approvers = [];
	...
	_issueTitle = "";
	_issueBody = "";
	_pollingIntervalSeconds = 10;
	_failOnDenial = true;
	_excludeWorkflowInitiator = false;
	_additionalApprovedWords = [];
	_additionalDeniedWords = [];
	constructor() {}
	static create() {
	...
	withPollingIntervalSeconds(seconds) {
		this._pollingIntervalSeconds = seconds;
		return this;
	}
	withFailOnDenial(fail) {
		this._failOnDenial = fail;
		return this;
	}
	withExcludeWorkflowInitiator(exclude) {
		this._excludeWorkflowInitiator = exclude;
		return this;
	}
	withAdditionalApprovedWords(words) {
		this._additionalApprovedWords = [...words];
		return this;
	}
	withAdditionalDeniedWords(words) {
	...
	build() {
		...
		if (this._approvers.length === 0) throw new Error("At least one approver is required");
		if (this._minimumApprovals > this._approvers.length) throw new Error(`Minimum approvals (${this._minimumApprovals}) exceeds approver count (${this._approvers.length})`);
		return new DeploymentGateService(this._token, this._owner, this._repo, this._approvers, this._minimumApprovals, this._issueTitle, this._issueBody, this._pollingIntervalSeconds, this._failOnDenial, this._excludeWorkflowInitiator, this._additionalApprovedWords, this._additionalDeniedWords);
	}
};
//#endregion
//#region src/tools/deployment-gate/settings.ts
/**
...
//#endregion
//#region src/tools/deployment-gate/runner.ts
/**
* Deployment gate runner
* Creates a GitHub issue for manual approval and polls for responses
*/
var DeploymentGateRunner = class extends RunnerBase {
	name = "deployment-gate";
	steps = new Map([["approve", this.runApprove.bind(this)]]);
	constructor() {
		super();
	}
	/**
	...
	*/
	async runApprove(agent) {
		try {
			const settings = getSettings(agent);
			const repoOwner = settings.targetRepositoryOwner || process.env.GITHUB_REPOSITORY_OWNER || "";
			const repoName = settings.targetRepository || process.env.GITHUB_REPOSITORY?.split("/")[1] || "";
			const runId = process.env.GITHUB_RUN_ID || "";
			if (!repoOwner || !repoName) return this.failure(/* @__PURE__ */ new Error("Could not determine target repository"));
			const issueTitle = settings.issueTitle || `Manual approval required for workflow run ${runId}`;
			agent.info("Starting deployment gate approval process...");
			const service = DeploymentGateBuilder.create().withToken(settings.secret).withOwner(repoOwner).withRepo(repoName).withApprovers(settings.approvers).withMinimumApprovals(settings.minimumApprovals).withIssueTitle(issueTitle).withIssueBody(settings.issueBody).withPollingIntervalSeconds(settings.pollingIntervalSeconds).withFailOnDenial(settings.failOnDenial).withExcludeWorkflowInitiator(settings.excludeWorkflowInitiator).withAdditionalApprovedWords(settings.additionalApprovedWords).withAdditionalDeniedWords(settings.additionalDeniedWords).build();
			agent.startGroup("Resolving approvers");
			const actor = settings.excludeWorkflowInitiator ? process.env.GITHUB_ACTOR : void 0;
			...
			agent.endGroup();
			if (resolvedApprovers.length === 0) return this.failure(/* @__PURE__ */ new Error("No approvers remaining after resolution"));
			...
			if (approvalStatus === ApprovalStatus.Approved) return this.success(outputs);
			if (settings.failOnDenial) return this.failure(/* @__PURE__ */ new Error("Deployment approval was denied"), outputs);
	...
	*/
	async pollForApproval(agent, service, issueNumber) {
		const intervalMs = service.pollingIntervalSeconds * 1e3;
		while (true) {
			...
			if (result.approvedBy.length > 0) agent.info(`Waiting for more approvals (${result.approvedBy.length}/${service.minimumApprovals || service.approvers.length})...`);
		}
	}
	/**
