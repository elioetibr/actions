...
class Logger {
  ...
  debug(message, context = {}) {
    coreExports.debug(this.formatMessage(message, context));
  }
  /**
   * Info level logging
   */
  info(message, context = {}) {
    coreExports.info(this.formatMessage(message, context));
  }
  /**
   * Warning level logging
   */
  warn(message, context = {}) {
    coreExports.warning(this.formatMessage(message, context));
  }
  /**
   * Error level logging
   */
  error(message, context = {}) {
    coreExports.error(this.formatMessage(message, context));
  }
}
...
const logger = createLogger$1("action");
...
function handleError(error, options = {}) {
  ...
  if (setFailed) {
    const failureMessage = isKnownError ? `Action failed with error: ${errorMessage}.` : `Unhandled error happened during execution: ${errorMessage}`;
    coreExports.setFailed(failureMessage);
  }
  if (rethrow) {
    throw error;
  }
}
function catchErrorAndSetFailed(error) {
...
console.log(semVerInfo.major);
console.log(semVerInfo.minor);
console.log(semVerInfo.patch);
console.log(semVerInfo.semVerSuffix);
console.log(semVerInfo.version);
console.log(semVerInfo.semVer);
...
console.log(version1.isGreaterThan(version2));
console.log(version1.isGreaterThan("1.5.0"));
console.log(version1.isCompatibleWith("2.0.0"));
console.log(version1.isEqualTo("2.1.0"));
const baseVersion = SemanticVersionBuilder.fromVersion("1.0.0").build().semVerInfo;
const majorIncrement = baseVersion.incrementMajor();
const minorIncrement = baseVersion.incrementMinor();
const patchIncrement = baseVersion.incrementPatch();
console.log(majorIncrement.semVer);
console.log(minorIncrement.semVer);
console.log(patchIncrement.semVer);
const releaseVersion = SemanticVersionBuilder.fromVersion("1.0.0").build().semVerInfo;
const alphaVersion = releaseVersion.withSuffix("alpha.1");
const betaVersion = releaseVersion.withSuffix("-beta.2");
const buildVersion = releaseVersion.withSuffix("+build.123");
console.log(alphaVersion.semVer);
console.log(betaVersion.semVer);
console.log(buildVersion.semVer);
const cleanVersion = alphaVersion.withoutSuffix();
console.log(cleanVersion.semVer);
const prereleaseVersion = SemanticVersionBuilder.fromVersion("2.0.0-rc.1+build.456").build().semVerInfo;
console.log(prereleaseVersion.isPrerelease());
console.log(prereleaseVersion.hasBuildMetadata());
const stableVersion = SemanticVersionBuilder.fromVersion("2.0.0").build().semVerInfo;
console.log(stableVersion.isPrerelease());
console.log(stableVersion.hasBuildMetadata());
const complexVersion = SemanticVersionBuilder.fromVersion("1.0.0").build().semVerInfo.incrementMinor().incrementPatch().withSuffix("beta.1");
console.log(complexVersion.semVer);
function checkVersionCompatibility(currentVersion, requiredVersion) {
...
console.log(`Version: ${versionForApi}`);
class ApiVersionService {
  ...
  isBreakingChangeFrom(otherVersion) {
...
console.log(apiService.getNextMinorVersion());
console.log(apiService.isBreakingChangeFrom("1.5.0"));
const versions = ["1.0.0", "1.2.0", "2.0.0-alpha", "2.1.0", "1.1.5"];
const parsedVersions = versions.map((v) => SemanticVersionBuilder.fromVersion(v).build().semVerInfo).sort((a, b) => {
...
function createGitHubContextService(context) {
...
function createServices(context$1 = context) {
  return new ServiceBuilder(context$1).buildAll();
}
...
class ActionRunner {
  ...
  async run() {
    try {
      coreExports.info("⭐ Starting Action Runner...");
      coreExports.info(`❓ Debug is Enabled: ${JSON.stringify(coreExports.isDebug())}`);
      coreExports.info(`❓ Debug is Enabled: ${JSON.stringify(this.services, null, 2)}`);
    } catch (error) {
      catchErrorAndSetFailed(error);
    }
  }
}
function run(context$1 = context, servicesContainer) {
  const services = createServices(context$1);
  const runner = new ActionRunner(services);
  return runner.run();
}
if (require.main === module) {
  run().then(handleSuccess).catch(catchErrorAndSetFailed);
}
